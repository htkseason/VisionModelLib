...
public class TextureModelTrain {
	...
	public static void train(String outputDir, double fractionRemain, int resolution_x, int resolution_y,
		...
		Mat delaunay_show = Mat.zeros(resolution_y, resolution_x, CvType.CV_64F);
		for (int i = 0; i < delaunay.length; i++) {
			int p1c = delaunay[i][0];
			int p2c = delaunay[i][1];
			int p3c = delaunay[i][2];
			double p1x = meanShape.get(p1c * 2, 0)[0];
			double p2x = meanShape.get(p2c * 2, 0)[0];
			double p3x = meanShape.get(p3c * 2, 0)[0];
			double p1y = meanShape.get(p1c * 2 + 1, 0)[0];
			double p2y = meanShape.get(p2c * 2 + 1, 0)[0];
			double p3y = meanShape.get(p3c * 2 + 1, 0)[0];
