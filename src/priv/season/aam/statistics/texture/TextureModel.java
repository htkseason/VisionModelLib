...
import priv.season.aam.util.Triangle;
...
public class TextureModel {
	...
	public static void printTo(Mat dst, Mat X, Mat pts) {
		Mat texture = X.reshape(1, (int) Math.sqrt(X_SIZE));

		AfflineTexture(texture, stdShape, dst, pts, delaunay);
	}
	...
	public static Mat getNormFace(Mat pic, Mat pts) {
		...
		double[] ptsdata = new double[pts.rows()];
		pts.get(0, 0, ptsdata);

		Mat result = Mat.zeros(new Size((int) Math.sqrt(X_SIZE), (int) Math.sqrt(X_SIZE)), pic.type());
